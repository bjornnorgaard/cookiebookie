...
<div class="navbar flex justify-between">
    <div class="">
        <div class="dropdown" class:hidden={!routes.length}>
            ...
            <ul class="menu menu-compact dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52" tabindex="-1">
                {#each routes as r}
                    <li>
                        <a href={r.route}
                           class={$page.url.pathname === r.route ? 'ml-1 bg-neutral text-neutral-content' : 'ml-1'}>{r.label}</a>
                    </li>
                {/each}
